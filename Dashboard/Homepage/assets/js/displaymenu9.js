function getMenu() {
    db.collection("Menu").where("Category", "==", "Drinks").onSnapshot(snapshot => {
        console.log();
        let items = [];
    ...
    items.forEach((item) => {
        ...
        <div class="product-grid-item mb-30">
            <div class="product-img mb-3">
                 <a data-id="${item.id}" class="menu-details" href="drink-hausboomlemonade.html"> 
                 <img src="${item.ImageUrl}" class=" w-100" alt="" id="menupic">
                 </a>
            </div>
            <div class="product-content">
                <h6 class="mb-101">RM${item.Price}</h6>
                 <a data-id="${item.id}" href="drink-hausboomlemonade.html">
                    <p>${item.Name}</p>
                 </a>
            </div>
